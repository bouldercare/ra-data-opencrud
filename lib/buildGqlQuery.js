...
export const buildArgs = (query, variables) => {
  ...
  return query.args.filter(arg => validVariables.includes(arg.name)).reduce((acc, arg) => [...acc, gqlTypes.argument(gqlTypes.name(arg.name), gqlTypes.variable(gqlTypes.name(arg.name)))], []);
};
...
export const buildApolloArgs = (query, variables) => {
  ...
  return query.args.filter(arg => validVariables.includes(arg.name)).reduce((acc, arg) => [...acc, gqlTypes.variableDefinition(gqlTypes.variable(gqlTypes.name(arg.name)), getArgType(arg))], []);
};
...
export default (introspectionResults => (resource, aorFetchType, queryType, variables, fragment) => {
  ...
  const fields = !!fragment ? buildFieldsFromFragment(fragment, resource.type.name, aorFetchType) : buildFields(introspectionResults)(resource.type.fields);
